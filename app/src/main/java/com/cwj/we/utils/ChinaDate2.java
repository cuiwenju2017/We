    ...
    public static String solarToLunar(String solarDate, Boolean isFormat) throws Exception {
        ...
        if (isFormat)
            if (lunarMonth > 9 && lunarDay > 9) {
                return lunarYear + "-" + (leapMonthFlag & (lunarMonth == leapMonth) ? "闰" : "") + lunarMonth + "-" + lunarDay;
            } else if (lunarMonth <= 9 && lunarDay <= 9) {
                return lunarYear + "-" + (leapMonthFlag & (lunarMonth == leapMonth) ? "闰" : "") + "0" + lunarMonth + "-" + "0" + lunarDay;
            } else if (lunarMonth <= 9 && lunarDay > 9) {
                return lunarYear + "-" + (leapMonthFlag & (lunarMonth == leapMonth) ? "闰" : "") + "0" + lunarMonth + "-" + lunarDay;
            } else if (lunarMonth > 9 && lunarDay <= 9) {
                return lunarYear + "-" + (leapMonthFlag & (lunarMonth == leapMonth) ? "闰" : "") + lunarMonth + "-" + "0" + lunarDay;
            }
        return "" + lunarYear + lunarMonth + lunarDay;
    }

    public static void main(String[] args) throws Exception {
        System.out.println(ChinaDate2.solarToLunar("1990-06-30", true));
        System.out.println(ChinaDate2.solarToLunar("1984-12-13", true));
        System.out.println(ChinaDate2.solarToLunar("1991-01-19", true));
    }
}
