...
public class MarketUtils {
    ...
    public boolean startMarket(Context mContext, String packageName) {
        try {
            ...
            ToastUtil.showTextToast(mContext, "要跳转的应用市场不存在!");
        } catch (Exception e) {
    ...
    /***
     * 指定包名，跳转到指定应用市场（需要传入应用包名，指定应用市场包名）
     * @param mContext
     * @param packageName
    ...
    public void openInstalledAppInURL(Context mContext, String packagename, String className, String urlStr) {
        try {
            ...
            ToastUtil.showTextToast(mContext, "要跳转的应用不存在!");
        } catch (Exception e) {
            ToastUtil.showTextToast(mContext, "" + e.getMessage());
        }
    }
    ...
    /****
     * 检查APP是否安装成功（需要传入应用包名）
     * @param packageName
     * @param context
     * @return
     */
    public boolean isInstalled(@NonNull String packageName, Context context) {
...
//        public static final String SUONI_PACKAGE_NAME = "com.android.vending";//索尼
    ...
    /**
     * 启动到应用商店app详情界面（需要传入应用包名，和应用商店包名）
     *
